...
쿼리 편집·결과 그리드 인라인 편집·스키마 관리·SSH 터널을 통합한 Wails v3 기반 네이티브 앱입니다.
...
- 다중 쿼리 동시 실행 (세미콜론 구분)
- 쿼리 실행 취소 (Stop 버튼)
- Placeholder 모달 — `?param` SQL 변수를 입력 폼으로 안전 치환
- 쿼리 탭 컨텍스트 메뉴 — 새 탭 / 모두 닫기 / 오른쪽 탭 닫기

### 결과 그리드
- 인라인 셀 편집 (INSERT / UPDATE / DELETE 자동 생성)
- 타입별 셀 에디터 — 날짜 DatePicker · Boolean 토글 · Numeric 검증 · ENUM 드롭다운 · SET 멀티셀렉트 · JSON 자동 포맷
- 신규 행 직접 입력 (INSERT), 명시적 NULL 설정 (`Ctrl+0`)
- 헤더에 PRIMARY KEY / UNIQUE / INDEX / FK 뱃지 자동 표시
...
- 외래 키(FK) 설정
- DDL 미리보기 및 실행
- CREATE 코드 탭 — `SHOW CREATE TABLE` 결과를 Monaco SQL 하이라이트로 표시 + 클립보드 복사
...
- 127.0.0.1 전용 listen, OS 키체인 보관 Bearer 토큰 인증
- 5 도구: list_connections / list_databases / list_tables / describe_table / execute_query
- 권한 게이트 3단계 (read-only 기본 / +쓰기 / +DDL)
- Connection allowlist (사용자가 노출할 연결 선택)
- AI 부트스트랩 프롬프트 + 클라이언트 설정 JSON 원클릭 복사
- 자가 헬스체크 + StatusBar 인디케이터 (재시작·테스트·설정 바로가기)
...
- 다크 모드 / 라이트 모드 전환
- 한국어 / 영어 UI 전환
- 메뉴바에 앱 버전 뱃지 표시 (macOS)
- ESC로 팝업 닫기, 키보드 단축키 설정
- 패널 크기 자유 조절 (리사이저)
- macOS 트래픽 라이트 / Windows 타이틀바 네이티브 통합
