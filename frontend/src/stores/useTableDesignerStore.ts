...
export const useTableDesignerStore = create<TableDesignerState>((set, get) => ({
  ...
  loadTable: async (connId, database, table) => {
    ...
    try {
      ...
      set({ loading: false })
      toast.error(`테이블 메타 로드 실패: ${e instanceof Error ? e.message : String(e)}`)
    }
  },
  ...
  buildAlter: async () => {
    ...
    if (mode === 'create') {
      ...
      try {
        ...
        toast.error(`CREATE 생성 실패: ${e instanceof Error ? e.message : String(e)}`)
        return null
      }
    }
    ...
    try {
      ...
      toast.error(`ALTER 생성 실패: ${e instanceof Error ? e.message : String(e)}`)
      return null
    }
  },
  ...
  save: async (connName) => {
    ...
    if (mode === 'create') {
      if (!editedMeta.name.trim()) { toast.error('테이블 이름을 입력하세요'); return }
      if (editedRows.length === 0) { toast.error('컬럼을 1개 이상 추가하세요'); return }
    } else if (!get().isDirty()) {
      toast('변경 사항이 없습니다')
      return
    }
    ...
    try {
      ...
      if (!stmt || !stmt.sql) {
        toast(mode === 'create' ? 'CREATE SQL 생성 실패' : '변경 사항이 없습니다')
        return
      }
      ...
      if (mode === 'create') {
        ...
        toast.success(`테이블 '${newName}' 생성 완료`)
        // SchemaTree 새로고침
