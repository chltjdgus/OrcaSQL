...
import { clearConnectionCache } from '@/lib/queryClient'
import { useTableViewStore } from '@/stores/useTableViewStore'
...
export const useConnectionStore = create<ConnectionStore>((set, get) => ({
  ...
  removeActiveConnection: (id) => {
    ...
    // Phase 61: 연결 해제 시 그 connId 의 세션 캐시(스키마/테이블 메타/데이터 쿼리 + 테이블 뷰
    // 상태)를 모두 제거해 메모리를 회수한다. gcTime: Infinity 라 명시적 제거가 없으면 누적된다.
    // 재접속은 새 connId 를 받으므로 어차피 fresh fetch — 끊긴 연결의 캐시는 보존 가치가 없다.
    clearConnectionCache(id)
    useTableViewStore.getState().clearForConn(id)
