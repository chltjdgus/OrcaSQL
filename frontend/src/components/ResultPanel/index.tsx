...
import { ChevronLeft, ChevronRight, ChevronsLeft, ChevronsRight, RefreshCw, Eraser, Loader2, ArrowUpDown, ArrowUp, ArrowDown } from 'lucide-react'
import ResultGrid from '@/components/ResultGrid'
import QueryHistory from '@/components/QueryHistory'
import TableInfoDesigner from '@/components/TableInfo'
import WhereFilterInput from './WhereFilterInput'
import { useThemeStore } from '@/stores/useThemeStore'
import { useConnectionStore } from '@/stores/connectionStore'
import { useLanguageStore } from '@/stores/useLanguageStore'
import { useTableViewStore, tableViewKey, DEFAULT_TABLE_VIEW } from '@/stores/useTableViewStore'
import { useWhereHistoryStore, whereHistoryKey } from '@/stores/useWhereHistoryStore'
import { t } from '@/i18n'
...
/** WhereHistory selector fallback — 참조 안정성 위해 모듈 상수 사용 */
...
function TableDataTab({
  ...
  // Phase 62: WHERE 검색 기록 — db.table 단위 영속(connId 무관, 재접속 후 유지)
  const histKey = whereHistoryKey(db, tableName)
  const whereHistory = useWhereHistoryStore((s) => s.histories[histKey]) ?? EMPTY_HISTORY
  const addWhereHistory = useWhereHistoryStore((s) => s.add)
  const removeWhereHistory = useWhereHistoryStore((s) => s.remove)
  const clearWhereHistory = useWhereHistoryStore((s) => s.clear)
  ...
  function applyWhere() {
    ...
    if (w) addWhereHistory(histKey, w)
  }

  /** 기록 항목 선택 — 입력에 채우고 즉시 적용(기록 최신으로 승격) */
  function pickHistory(where: string) {
    patchView(viewKey, { page: 0, whereInput: where, activeWhere: where })
    addWhereHistory(histKey, where)
  }
  ...
  return (
    <div className="osql-table-data-tab flex flex-col h-full overflow-hidden">
      ...
      <div className={`osql-table-data-toolbar flex items-center gap-2 px-2 py-1 border-b shrink-0 flex-wrap ${toolbarCls}`}>
        {/* WHERE 필터 — 컬럼명 자동완성 + 검색 기록 (Phase 62) */}
        <WhereFilterInput
          value={whereInput}
          onChange={(v) => patchView(viewKey, { whereInput: v })}
          onApply={applyWhere}
          onClear={() => patchView(viewKey, { whereInput: '', activeWhere: '', page: 0 })}
          columns={columns}
          history={whereHistory}
          onPickHistory={pickHistory}
          onRemoveHistory={(w) => removeWhereHistory(histKey, w)}
          onClearHistory={() => clearWhereHistory(histKey)}
          isDark={isDark}
          language={language}
        />
