...
export default function TableDesigner({ connId, database, table, onClose }: Props) {
  ...
  useEffect(() => {
    ...
    GenerateAlterSQL(database, table, original, current)
      .then((stmt) => setAlterSQL(stmt.sql || '-- 변경 없음'))
      .catch(() => setAlterSQL('-- SQL 생성 실패'))
  }, [current, original, database, table])
  ...
  if (loading || !current) {
    return (
      <div className="flex items-center justify-center h-full text-[var(--color-text-muted)] text-sm">
        <RefreshCw size={16} className="animate-spin mr-2" /> 테이블 구조 로드 중...
      </div>
    )
  }
  ...
  return (
    <div className="flex flex-col h-full bg-[var(--color-bg-primary)] text-[var(--color-text-primary)]">
      ...
      <div className="flex border-b border-[var(--color-border)] bg-[var(--color-bg-secondary)] shrink-0">
        {(['columns', 'indexes', 'foreignkeys', 'options', 'ddl'] as Tab[]).map((t) => (
          <button
            key={t}
    ...
    <div className="flex h-full overflow-hidden">
      ...
      <div className="w-56 border-r border-[var(--color-border)] flex flex-col overflow-hidden">
        <div className="flex items-center justify-between px-2 py-1 bg-[var(--color-bg-secondary)] border-b border-[var(--color-border)] shrink-0">
          <span className="text-[10px] text-[var(--color-text-muted)]">컬럼 목록</span>
          <button
            onClick={addColumn}
...
function IndexesTab({
  ...
  const ctxItems: ContextMenuOption[] = contextMenu
    ? [
        { label: '인덱스 추가', icon: <Plus size={12} />, onClick: addIndex },
        { separator: true },
        {
          label: '컬럼 모두 제거 (초기화)',
  ...
  return (
    <div className="p-4 overflow-y-auto h-full">
      ...
      <div className="flex items-center justify-between mb-3">
        <span className="text-xs text-[var(--color-text-muted)]">인덱스 ({indexes.length})</span>
        <button
          onClick={addIndex}
      ...
      {indexes.length === 0 && (
        <div className="text-center text-[var(--color-text-muted)] text-xs mt-8">인덱스 없음</div>
      )}
                ...
                <div className="px-4 pb-4 border-t border-[var(--color-border)] pt-3 space-y-3">
                  ...
                  <div>
                    <div className="text-[10px] text-[var(--color-text-muted)] mb-1.5">
                      인덱스 컬럼 순서 <span className="text-[var(--color-null)]">(드래그로 순서 변경)</span>
                    </div>
                    {idx.columns.length === 0 && (
                      <div className="text-[10px] text-[var(--color-null)] italic py-1 pl-1">컬럼이 없습니다</div>
                    )}
                    <div className="space-y-1">
                  ...
                  {!idx.isPrimary && availableCols.length > 0 && (
                    <div>
                      <div className="text-[10px] text-[var(--color-text-muted)] mb-1.5">컬럼 추가</div>
...
function ForeignKeysTab({
  ...
  return (
    <div className="p-4 overflow-y-auto h-full">
      <div className="flex items-center justify-between mb-3">
        <span className="text-xs text-[var(--color-text-muted)]">외래 키</span>
        <button
          onClick={add}
          className="flex items-center gap-1 px-2 py-1 text-xs rounded border border-[var(--color-border)] text-[var(--color-text-subtle)] hover:text-[var(--color-text-primary)] hover:bg-[var(--color-bg-hover)] transition-colors"
        >
      ...
      {fks.length === 0 && (
        <div className="text-center text-[var(--color-text-muted)] text-xs mt-8">외래 키 없음</div>
      )}
