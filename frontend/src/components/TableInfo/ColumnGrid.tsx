...
import { Plus, Minus, ChevronUp, ChevronDown, ArrowUpDown, AlertTriangle, Search, X } from 'lucide-react'
...
export default function ColumnGrid({ database, table, onInsertSQL }: Props) {
  ...
  // Phase 62: 컬럼명 필터 (표시 전용 — 편집/선택은 rowId 기준이라 영향 없음)
  ...
  // 시각적 필터 + 정렬: 원본 editedRows 복사본에 컬럼명 필터·정렬 적용
  // (ordinalPos·rowId 는 그대로 유지 — 표시 기준만 바뀜)
  const displayRows = useMemo(() => {
    const q = nameFilter.trim().toLowerCase()
    let rows = q ? editedRows.filter((r) => r.name.toLowerCase().includes(q)) : editedRows
    if (sortKey && sortDir) {
      rows = [...rows].sort((a, b) => {
    ...
    return rows
  }, [editedRows, sortKey, sortDir, nameFilter])
  ...
  return (
    <div className="osql-columngrid flex flex-col h-full overflow-hidden bg-[var(--color-bg-primary)]">
      ...
      <div className="osql-columngrid-toolbar flex items-center gap-1 px-2 py-1 border-b border-[var(--color-border)] bg-[var(--color-bg-secondary)] shrink-0">
        ...
        {/* 컬럼명 필터 (Phase 62) */}
        <div className="osql-columngrid-filter ml-auto flex items-center gap-1 w-44 px-2 py-0.5 rounded border border-[var(--color-border)] bg-[var(--color-bg-primary)] focus-within:border-[var(--color-accent)] transition-colors">
          <Search size={11} className="text-[var(--color-null)] shrink-0" />
          <input
            value={nameFilter}
            onChange={(e) => setNameFilter(e.target.value)}
            placeholder={t('cgFilterPh', language)}
            spellCheck={false}
            className="osql-columngrid-filter-input flex-1 min-w-0 bg-transparent text-[10px] text-[var(--color-text-primary)] outline-none placeholder-[var(--color-null)]"
          />
          {nameFilter && (
            <button
              onClick={() => setNameFilter('')}
              title={t('cgFilterClear', language)}
              className="text-[var(--color-null)] hover:text-[var(--color-text-primary)] transition-colors shrink-0"
            >
              <X size={10} />
            </button>
          )}
        ...
        <span className="text-[9px] text-[var(--color-null)] shrink-0">
          {nameFilter.trim()
            ? `${displayRows.length}${t('cgFilterCountSuffix', language)} / ${editedRows.length}`
            : language === 'ko'
      ...
      <div className="flex-1 overflow-auto" onContextMenu={(e) => e.preventDefault()}>
        ...
        {editedRows.length > 0 && displayRows.length === 0 && (
          <div className="flex items-center justify-center h-20 text-[var(--color-null)] text-xs">
            {language === 'ko' ? `"${nameFilter}" 와 일치하는 컬럼 없음` : `No columns match "${nameFilter}"`}
          </div>
        )}
      </div>
