...
const GROUPED_TYPES = [
  { group: '정수형',    types: ['TINYINT', 'SMALLINT', 'MEDIUMINT', 'INT', 'BIGINT'] },
  { group: '실수형',    types: ['FLOAT', 'DOUBLE', 'DECIMAL', 'NUMERIC'] },
  { group: '문자열',    types: ['CHAR', 'VARCHAR', 'TINYTEXT', 'TEXT', 'MEDIUMTEXT', 'LONGTEXT'] },
  { group: '이진/블롭', types: ['BINARY', 'VARBINARY', 'TINYBLOB', 'BLOB', 'MEDIUMBLOB', 'LONGBLOB'] },
  { group: '날짜/시간', types: ['DATE', 'DATETIME', 'TIMESTAMP', 'TIME', 'YEAR'] },
  { group: '기타',      types: ['BOOLEAN', 'BIT', 'ENUM', 'SET', 'JSON', 'UUID'] },
]
...
const DEFAULT_COLUMNS: ColumnMeta[] = [
  { key: 'ordinal',   label: '#',         width: 36,  align: 'center' },
  { key: 'flags',     label: '키',        width: 50,  align: 'center' },
  { key: 'name',      label: '컬럼명',    width: 160, align: 'left' },
  { key: 'type',      label: '데이터형',  width: 110, align: 'left' },
  { key: 'length',    label: '길이',      width: 80,  align: 'left' },
  { key: 'unsigned',  label: '부호없음',  width: 70,  align: 'center' },
  { key: 'nullable',  label: 'NULL',      width: 60,  align: 'center' },
  { key: 'zerofill',  label: '0채움',    width: 60,  align: 'center' },
  { key: 'default',   label: '기본값',    width: 160, align: 'left' },
  { key: 'collation', label: '콜레이션',  width: 140, align: 'left' },
  { key: 'comment',   label: '코멘트',    width: 180, align: 'left' },
]
...
export default function ColumnGrid({ database, table, onInsertSQL }: Props) {
  ...
  const copySelectedAsTSV = useCallback(() => {
    ...
    toast.success(`${rows.length}개 컬럼 복사됨`)
    ...
    <div className="osql-columngrid flex flex-col h-full overflow-hidden bg-[var(--color-bg-primary)]">
      ...
      {headerCtx && (
        <ContextMenu x={headerCtx.x} y={headerCtx.y} onClose={() => setHeaderCtx(null)}>
          <CtxItem onClick={showAllColumns}>모든 열 보이기</CtxItem>
          <CtxItem
            onClick={() => {
              void navigator.clipboard.writeText(headerCtx.key)
              toast.success('열 키 복사됨')
            }}
          >
            이 열 키 복사
          </CtxItem>
      ...
      {rowCtx && (
        <ContextMenu x={rowCtx.x} y={rowCtx.y} onClose={() => setRowCtx(null)}>
          <CtxItem onClick={copySelectedAsTSV}>복사 (TSV)</CtxItem>
          <CtxDivider />
          <CtxItem onClick={addRowAfterSelected}>행 추가</CtxItem>
          <CtxItem onClick={deleteSelected}>행 제거</CtxItem>
          <CtxDivider />
          <CtxItem onClick={() => moveSelected(-1)}>위로 이동</CtxItem>
          <CtxItem onClick={() => moveSelected(1)}>아래로 이동</CtxItem>
          <CtxDivider />
          <CtxSubLabel>새 인덱스 생성</CtxSubLabel>
          ...
          <CtxDivider />
          <CtxItem onClick={insertAlterAddColumnSQL}>ALTER ADD COLUMN 쿼리 생성 → 에디터</CtxItem>
        </ContextMenu>
      )}
    </div>
  )
...
function EnumEditor({ value, onChange }: { value: string; onChange: (v: string) => void }) {
  ...
  const summary = value ? `(${value.split(',').length}개)` : '편집...'
  ...
  return (
    <div ref={containerRef}>
      ...
      {open && (
        ...
        >
          <div className="text-[9px] text-[var(--color-null)] mb-1">한 줄 = 한 값</div>
          <textarea
            value={draft}
...
/** 데이터형별 기본값 프리셋 목록 */
function getPresets(dataType: string, rowId: string, allRows: ColumnRow[]): DefaultPreset[] {
...
function DefaultEditor({ row, allRows, onPatch }: {
  ...
  const presets = getPresets(row.dataType, row.rowId, allRows)
  ...
  return (
    <div ref={containerRef} className="flex items-center w-full gap-0.5">
      ...
      {open && (
        ...
        >
          {/* 기본값 프리셋 */}
          <div className="px-3 pb-0.5 text-[9px] uppercase text-[var(--color-null)]">기본값</div>
          {presets.map((p) => (
            <button
              key={p.label}
