...
export default function ERDiagram({ connId, database, onClose }: Props) {
  ...
  const loadSchema = useCallback(async () => {
    ...
    try {
      ...
      setNodes(newNodes)
      setEdges(newEdges)
      const fkMsg = fks.length > 0 ? ` · FK ${fks.length}개` : ' · FK 없음'
      toast.success(`${baseTables.length}개 테이블 로드 완료${fkMsg}`)
    } catch (e) {
      toast.error(`스키마 로드 실패: ${e}`)
    } finally {
      setLoading(false)
    }
  }, [connId, database, setNodes, setEdges])

  useEffect(() => { loadSchema() }, [loadSchema])

  function exportSVG() {
    const svg = document.querySelector('.react-flow__renderer svg') as SVGElement | null
    if (!svg) { toast.error('SVG 내보내기 실패'); return }
      ...
      <div className="flex-1 overflow-hidden">
        {loading && nodes.length === 0 ? (
          <div className="flex items-center justify-center h-full text-[var(--color-text-muted)] text-sm gap-2">
            <RefreshCw size={16} className="animate-spin" /> 스키마 로드 중...
          </div>
        ) : (
          <ReactFlow
            nodes={nodes}
