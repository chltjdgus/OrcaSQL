...
function SessionForm({
  ...
  async function handleTest() {
    ...
    try {
      ...
      if (r.ok) {
        ...
        setAvailableDbs(dbs)
        toast.success(t('cmTestOk', language))
      } else {
        toast.error(r.message)
      }
    } catch (e) {
      toast.error(`${t('smTestFailPrefix', language)}${e}`)
    } finally {
      setTesting(false)
    }
  }
  ...
  async function handleConnect() {
    if (!form.name.trim()) { toast.error(t('smEnterName', language)); return }
    if (!form.host.trim()) { toast.error(t('smEnterHost', language)); return }
    setConnecting(true)
    try {
      ...
      toast.success(`${toSave.name} ${t('toastConnectedSuffix', language)}`)
      logMsg({ kind: 'connection', level: 'success', title: `${t('cpLogConnectedPrefix', language)}${toSave.name}`, connName: toSave.name })
    } catch (e) {
      const errMsg = e instanceof Error ? e.message : String(e)
      toast.error(`${t('cpConnFailPrefix', language)}${errMsg}`)
      logMsg({ kind: 'connection', level: 'error', title: `${t('cpConnFailPrefix', language)}${form.name}`, detail: errMsg, connName: form.name })
    } finally {
      setConnecting(false)
    }
  }
  ...
  return (
    <div className="flex flex-col h-full">
      <div className="flex-1 overflow-y-auto p-4 space-y-3">
        ...
        <section>
          ...
          <div className="space-y-2">
            ...
            <div className="flex gap-2">
              ...
              <div className="w-24">
                <label className={labelCls}>{t('smCharset', language)}</label>
            ...
            <div className="flex items-center gap-2">
              ...
              <label htmlFor="tls" className="text-xs text-[var(--color-text-subtle)]">{t('smUseTls', language)}</label>
            </div>
          </div>
        </section>
        ...
        {/* 색상 */}
        <section>
          <div className="text-[11px] font-semibold text-[var(--color-accent)] mb-1 uppercase tracking-wide">{t('smSessionColor', language)}</div>
        ...
        <section>
          <div className="flex items-center gap-2 mb-2">
            ...
            <label htmlFor="useSSH" className="text-[11px] font-semibold text-[var(--color-accent)] uppercase tracking-wide">{t('smSshTunnel', language)}</label>
          </div>
          {form.useSSH && (
            <div className="space-y-2 pl-2 border-l border-[var(--color-border)]">
              <div className="flex gap-2">
                <div className="flex-1">
                  <label className={labelCls}>{t('smSshHost', language)}</label>
        ...
        <section>
          <div className="flex items-center gap-2 mb-2">
            ...
            <label htmlFor="useProxy" className="text-[11px] font-semibold text-[var(--color-accent)] uppercase tracking-wide">{t('smProxy', language)}</label>
          </div>
          {form.useProxy && (
            <div className="space-y-2 pl-2 border-l border-[var(--color-border)]">
              <div className="flex gap-2 items-end">
                <div>
                  <label className={labelCls}>{t('smType', language)}</label>
      ...
      <div className="shrink-0 p-3 border-t border-[var(--color-border)] flex items-center gap-2">
        {testResult && (
          <span className={`text-[10px] flex-1 ${testResult.ok ? 'text-[var(--color-success)]' : 'text-[var(--color-error)]'}`}>
            {testResult.ok ? `✓ ${t('cmTestOk', language)} (${testResult.serverVer})` : `✗ ${testResult.message}`}
          </span>
        )}
        ...
        >
          {testing ? t('cmTesting', language) : t('cmTestBtn', language)}
        </button>
        <button
          onClick={onCancel}
        ...
        <button
          ...
          title={t('smConnectTitle', language)}
        >
          {connecting ? t('smConnecting', language) : t('smConnect', language)}
        </button>
      </div>
    </div>
  )
  ...
  return (
    <div className="fixed inset-0 z-[300] flex items-center justify-center bg-black/60">
      <div className="bg-[var(--color-bg-secondary)] border border-[var(--color-border)] rounded-lg p-5 w-80 shadow-2xl">
        <div className="text-sm font-semibold text-[var(--color-text-primary)] mb-2">{t('smDeleteConfirm', language)}</div>
        {isGroup ? (
          <div className="text-xs text-[var(--color-text-subtle)] mb-4">
            {language === 'ko' ? (
              ...
              <>Delete group <span className="font-medium text-[var(--color-error)]">{targetName}</span>.</>
            )}
            {childCount > 0 && (
              <div className="mt-1">
                {language === 'ko' ? (
                  <>하위에 <span className="font-medium text-[var(--color-warning)]">{childCount}개</span>의 세션이 있습니다. 함께 삭제하시겠습니까?</>
                ) : (
                  <>This group contains <span className="font-medium text-[var(--color-warning)]">{childCount}</span> session(s). Delete them too?</>
                )}
              </div>
            )}
          </div>
          ...
          <div className="text-xs text-[var(--color-text-subtle)] mb-4">
            {language === 'ko' ? (
              ...
              <>Delete session <span className="font-medium text-[var(--color-error)]">{targetName}</span>?</>
            )}
          </div>
        )}
        <div className="flex justify-end gap-2">
          ...
          >
            {isGroup && childCount > 0 ? t('smDeleteAll', language) : t('smDelete', language)}
          </button>
        </div>
      </div>
    </div>
...
export default function SessionManager({
  ...
  async function handleNewGroup() {
    ...
    const newGroup: SessionGroup = {
      ...
      name: t('smNewGroupName', language),
      color: '',
      parentId,
  ...
  async function handleDuplicate(id: string, kind: 'session' | 'group') {
    if (kind === 'session') {
      ...
      if (!orig) return
      const dup: ConnectConfig = { ...orig, id: crypto.randomUUID(), name: `${orig.name}${t('smCopySuffix', language)}`, sortOrder: (orig.sortOrder ?? 0) + 1 }
      await SaveConnection(dup)
    } else {
      const orig = groups.find((g) => g.id === id)
      if (!orig) return
      const dup: SessionGroup = { ...orig, id: crypto.randomUUID(), name: `${orig.name}${t('smCopySuffix', language)}`, order: orig.order + 1 }
    ...
    >
      ...
      >
        ...
        <div className="flex items-center justify-between px-4 py-2.5 border-b border-[var(--color-border)] bg-[var(--color-bg-secondary)] shrink-0">
          <span className="text-sm font-semibold text-[var(--color-text-primary)]">{t('smTitle', language)}</span>
        ...
        <div className="flex flex-1 overflow-hidden">
          ...
          <div className="w-72 flex flex-col border-r border-[var(--color-border)] bg-[var(--color-bg-deep)]">
            ...
            <div className="text-[9px] text-[var(--color-null)] px-3 py-0.5">
              {t('smSortLabel', language)}{sortMode === 'name' ? t('smSortName', language) : sortMode === 'host' ? t('smHost', language) : t('smSortLastUsed', language)} {sortAsc ? '↑' : '↓'}
            </div>
            ...
            <div className="flex-1 overflow-y-auto py-1">
              {tree.length === 0 && (
                <div className="text-[11px] text-[var(--color-null)] text-center py-8">
                  {t('smNoSessions', language)}<br />
                  <span className="text-[10px]">{t('smAddBelow', language)}</span>
                </div>
              )}
            ...
            <div className="shrink-0 border-t border-[var(--color-border)] p-2 flex gap-1">
              ...
              >
                <Plus size={12} /> {t('smNewSession', language)}
              </button>
              <button
              ...
              >
                <FolderPlus size={12} /> {t('smNewGroupName', language)}
              </button>
            </div>
          </div>
          ...
          <div className="flex-1 overflow-hidden">
            {formCfg ? (
              ...
              <div className="flex items-center justify-center h-full text-[var(--color-null)] text-sm flex-col gap-2">
                ...
                <span>{t('smSelectOrAdd', language)}</span>
              </div>
            )}
          </div>
        </div>
