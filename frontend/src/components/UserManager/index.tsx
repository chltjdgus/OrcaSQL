      ...
      <div className="w-56 shrink-0 flex flex-col border-r border-[var(--color-border)]">
        <div className="flex items-center justify-between px-3 py-2 border-b border-[var(--color-border)] shrink-0">
          <div className="flex items-center gap-2">
            ...
            <span className="text-xs font-medium text-[var(--color-text-subtle)]">사용자</span>
          </div>
        ...
        <div className="flex-1 overflow-y-auto">
          {isLoading ? (
            <div className="px-3 py-4 text-[10px] text-[var(--color-null)]">로딩 중...</div>
          ) : (
            users.map((u) => (
              <div
                key={`${u.user}@${u.host}`}
      ...
      <div className="flex-1 overflow-hidden">
        {showCreate ? (
          ...
          <div className="flex flex-col items-center justify-center h-full text-[var(--color-null)]">
            <Users size={24} />
            <p className="mt-2 text-xs">좌측에서 사용자를 선택하세요</p>
          </div>
        )}
      </div>
    </div>
...
function CreateUserForm({
  ...
  const createMut = useMutation({
    ...
    onSuccess: () => { toast.success(`'${user}'@'${host}' 생성 완료`); onDone() },
    onError: (e) => toast.error(`생성 실패: ${e}`),
  })
  ...
  return (
    <div className="p-4 space-y-3 max-w-md">
      <div className="flex items-center gap-2 mb-2">
        ...
        <span className="text-xs font-medium text-[var(--color-text-subtle)]">새 사용자 생성</span>
      </div>
...
function UserDetail({ connId, user }: { connId: string; user: UserRow }) {
  ...
  const lockMut = useMutation({
    ...
    onSuccess: (_, lock) => {
      toast.success(lock ? '계정 잠금 완료' : '계정 잠금 해제 완료')
      qc.invalidateQueries({ queryKey: ['users', connId] })
    },
    onError: (e) => toast.error(`실패: ${e}`),
  })
      ...
      <section className="bg-[var(--color-bg-secondary)] rounded border border-[var(--color-border)] p-3 space-y-1.5">
        <div className="flex items-center justify-between mb-2">
          <div className="flex items-center gap-1.5">
            ...
            <span className="text-xs font-medium text-[var(--color-text-subtle)]">사용자 정보</span>
          </div>
          <div className="flex items-center gap-1">
            <button
        ...
        {grantsOpen && (
          <div className="border-t border-[var(--color-border)]">
            ...
            {grantsQuery.isLoading ? (
              <div className="px-3 py-3 text-[10px] text-[var(--color-null)]">로딩 중...</div>
            ) : (
...
function ChangePasswordForm({
  ...
  const mut = useMutation({
    ...
    onSuccess: () => { toast.success('비밀번호 변경 완료'); onDone() },
    onError: (e) => toast.error(`변경 실패: ${e}`),
  })

  return (
    <div className="mt-2 pt-2 border-t border-[var(--color-border)] space-y-2">
      <span className="text-[10px] text-[var(--color-text-subtle)] font-medium">새 비밀번호</span>
      <FieldRow label="">
        <div className="relative flex-1">
          <input
            type={showPw ? 'text' : 'password'}
...
function GrantForm({
  ...
  const mut = useMutation({
    mutationFn: mutFn,
    onSuccess: () => {
      toast.success(revokeMode ? 'REVOKE 완료' : 'GRANT 완료')
