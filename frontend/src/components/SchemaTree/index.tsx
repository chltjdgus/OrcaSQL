...
/**
 * Phase 61: 세션 단위 캐싱. 한 번 가져온 스키마는 연결 세션 동안 그대로 유지한다
 * (시간 만료/GC 없음). 새로고침(handleRefresh)·DDL 감지·새 연결 시에만 다시 가져온다.
 * 전역 기본값(`lib/queryClient`)도 Infinity 라 사실상 상속이지만, 의도를 명시하고
 * 각 useQuery 호출의 가독성을 위해 상수를 유지한다.
 */
const SCHEMA_STALE = Infinity
const SCHEMA_GC    = Infinity
const COL_STALE    = Infinity
import {
 ...
 */
/** 스키마/구조 캐시를 한 번에 무효화하는 쿼리 키 목록 ('tableMeta' = 디자이너 메타) */
const SCHEMA_KEYS = ['databases', 'tables', 'columns', 'views', 'procedures', 'functions', 'triggers', 'events', 'tableMeta'] as const
/** Phase 61: 새로고침 시 함께 무효화할 테이블 데이터 캐시 — 열려있는 Data 탭을 즉시 재조회시킨다. */
const DATA_KEYS = ['tableData', 'tableCount'] as const
...
export default function SchemaTree({ onTableSelect, onNameDoubleClick, onOpenTableDirectly, onOpenNewTable, initialExpanded }: SchemaTreeProps) {
  ...
  /**
   * Phase 61: 새로고침 = "이 시점에 캐시를 버리고 다시 가져온다" 의 유일한 진입점.
   * 스키마/구조(SCHEMA_KEYS) + 열려있는 테이블 데이터(DATA_KEYS) 를 모두 무효화한다.
   * active observer(보이는 트리·그리드)는 즉시 refetch, inactive 캐시는 stale 로 표시돼
   * 다음에 다시 열 때 fresh fetch 된다.
   */
  const handleRefresh = useCallback(() => {
    startRefreshTransition(async () => {
      await Promise.all(
        [...SCHEMA_KEYS, ...DATA_KEYS].map((key) => queryClient.invalidateQueries({ queryKey: [key] }))
      )
      toast.success(t('stRefreshDone', language))
    })
  }, [queryClient, startRefreshTransition, language])
        ...
        <div className={`osql-schema-tree-search px-2 py-1.5 border-b shrink-0 ${isDark ? 'border-[#2d3748]' : 'border-[#d1d5db]'}`}>
          <div className="flex items-center gap-1">
            ...
            <button
              ...
              title={t('stRefreshTitle', language)}
...
function DatabaseNode({
  ...
  async function handleCreateDatabase() {
    const name = prompt(t('stNewDbPrompt', language))
    if (!name) return
    try {
      await CreateDatabase(connId, name)
      toast.success(language === 'ko' ? `데이터베이스 '${name}' 생성 완료` : `Database '${name}' created`)
      queryClient.invalidateQueries({ queryKey: ['databases', connId] })
    } catch (e) {
      toast.error(`${t('stCreateFailPrefix', language)}${e}`)
    }
  }

  async function handleDropDatabase() {
    const input = prompt(language === 'ko'
      ? `DROP DATABASE를 실행하려면 DB명 '${db}'을 입력하세요:`
      : `Type the database name '${db}' to run DROP DATABASE:`)
    if (input !== db) { toast.error(t('stCancelDbMismatch', language)); return }
    try {
      await DropDatabase(connId, db)
      toast.success(language === 'ko' ? `데이터베이스 '${db}' 삭제 완료` : `Database '${db}' deleted`)
      queryClient.invalidateQueries({ queryKey: ['databases', connId] })
    } catch (e) {
      toast.error(`${t('stDeleteFailPrefix', language)}${e}`)
    }
  }

  const dbContextItems: ContextMenuOption[] = [
    {
      label: t('stCopyDbName', language),
      icon: <Copy size={11} />,
      onClick: () => { navigator.clipboard.writeText(`\`${db}\``); toast.success(t('stCopied', language)) },
    },
    { separator: true },
    {
      label: t('stNewTableDesigner', language),
      icon: <Plus size={11} className="text-[#68d391]" />,
      onClick: () => onOpenNewTable(connId, db),
    },
    {
      label: t('stSchemaRefresh', language),
      icon: <RefreshCw size={11} />,
      onClick: () => {
        queryClient.invalidateQueries({ queryKey: ['tables', connId, db] })
        toast.success(language === 'ko' ? `'${db}' 새로고침 완료` : `'${db}' refreshed`)
      },
    },
    { separator: true },
    {
      label: t('stCreateDb', language),
        ...
        <>
          ...
          {(!isSearching || baseTableList.length > 0) && (
            <CategoryNode
              connId={connId} db={db}
              category="tables"
              label={`${t('stTables', language)} (${baseTableList.length})`}
          ...
          {(!isSearching || viewList.length > 0) && (
            <CategoryNode
              connId={connId} db={db}
              category="views"
              label={`${t('stViews', language)} (${viewList.length})`}
...
function CategoryNode({
  ...
  const createMenu: ContextMenuOption[] =
    category === 'tables'
      ? [{
          label: t('stCreateTable', language),
          icon: <Plus size={11} className="text-[#4299e1]" />,
          onClick: () => onOpenNewTable(connId, db),
        }]
      : category === 'views'
      ? [{
          label: t('stCreateView', language),
          icon: <Plus size={11} className="text-[#68d391]" />,
          onClick: () =>
            appendOrNewTab(
...
function StoredObjectCategory({
  ...
  const createMenu: ContextMenuOption[] =
    category === 'procedures'
      ? [{
          label: t('stCreateProcedure', language),
      ...
      : category === 'functions'
      ? [{
          label: t('stCreateFunction', language),
      ...
      : category === 'events'
      ? [{
          label: t('stCreateEvent', language),
...
function StoredObjectNode({
  ...
  async function showDDL() {
    try {
      ...
      toast.error(t('stDdlFail', language))
    }
  }

  const contextItems: ContextMenuOption[] = [
    {
      label: t('stShowDdl', language),
      icon: <Code size={11} />,
      onClick: showDDL,
    },
    {
      label: t('stCopyName', language),
      icon: <Copy size={11} />,
      onClick: () => {
        navigator.clipboard.writeText(obj.name)
        toast.success(t('stNameCopied', language))
      },
    },
    ...(category === 'procedures'
      ? [{ separator: true as const }, {
          label: t('stRunCall', language),
          icon: <Play size={11} />,
...
function TableNode({ connId, db, table, expandedSet, toggleKey, onOpenImport, onTableSelect, onNameDoubleClick, onOpenTableDirectly, searchQuery = '', showRowCount = false, selectedNodeKey, setSelectedNodeKey }: TableNodeProps) {
  ...
  const { selectedConnId, addTab, setSelectedConn, setSelectedDatabase } = useConnectionStore()
  ...
  async function truncateTable() {
    ...
    try {
      ...
      toast.success(`${table.name} ${t('stTruncateDoneSuffix', language)}`)
    } catch (e) {
      toast.error(`${t('stTruncateFailPrefix', language)}${e}`)
    }
  }

  async function dropTable() {
    const input = prompt(language === 'ko'
      ? `DROP TABLE을 실행하려면 테이블명 '${table.name}'을 입력하세요:`
      : `Type the table name '${table.name}' to run DROP TABLE:`)
    if (input !== table.name) { toast.error(t('stCancelTableMismatch', language)); return }
    try {
      await runLoggedQuery({
        connId,
        database: db,
      ...
      toast.success(`${table.name} ${t('stDropDoneSuffix', language)}`)
      queryClient.invalidateQueries({ queryKey: ['tables', connId, db] })
    } catch (e) {
      toast.error(`${t('stDropFailPrefix', language)}${e}`)
    }
  }

  async function renameTable() {
    const newName = prompt(t('stNewTableNamePrompt', language), table.name)
    if (!newName || newName === table.name) return
    try {
      await RenameTable(connId, db, table.name, newName)
      toast.success(language === 'ko' ? `${table.name} → ${newName} 이름 변경 완료` : `Renamed ${table.name} → ${newName}`)
      queryClient.invalidateQueries({ queryKey: ['tables', connId, db] })
    } catch (e) {
      toast.error(`${t('stRenameFailPrefix', language)}${e}`)
    }
  }

  async function copyTableStructure() {
    const newName = prompt(t('stCopyTableNamePrompt', language), `${table.name}_copy`)
    if (!newName) return
    try {
      await CopyTable(connId, db, table.name, newName, false)
      toast.success(language === 'ko' ? `${table.name} → ${newName} 구조 복사 완료` : `Copied structure ${table.name} → ${newName}`)
      queryClient.invalidateQueries({ queryKey: ['tables', connId, db] })
    } catch (e) {
      toast.error(`${t('stCopyFailPrefix', language)}${e}`)
    }
  }
  ...
  async function copyTableWithData() {
    ...
    try {
      ...
      toast.success(language === 'ko' ? `${table.name} → ${newName} 데이터 포함 복사 완료` : `Copied with data ${table.name} → ${newName}`)
      queryClient.invalidateQueries({ queryKey: ['tables', connId, db] })
    } catch (e) {
      toast.error(`${t('stCopyFailPrefix', language)}${e}`)
    }
  ...
  async function exportData(format: 'csv' | 'json' | 'sql') {
    try {
      ...
      toast.success(`${table.name}.${format}${t('stExportDoneSuffix', language)}`)
    } catch (e) {
      toast.error(`${t('stExportFailPrefix', language)}${e}`)
    }
  }
  ...
  const contextItems: ContextMenuOption[] = [
    ...
    { separator: true },
    {
      label: t('stCopyTableName', language),
      icon: <Copy size={11} />,
      onClick: () => {
        navigator.clipboard.writeText(`\`${db}\`.\`${table.name}\``)
        toast.success(t('stTableNameCopied', language))
      },
    },
    { separator: true as const },
    {
      // BugFix-DL: Data 탭에 저장된 필터·정렬·페이지 상태를 초기화
      label: t('tableDataResetView', language),
      icon: <Eraser size={11} className="text-[#a0aec0]" />,
      onClick: () => {
        resetTableView(tableViewKey(connId, db, table.name))
        toast.success(t('tableViewResetDone', language))
      },
    },
    { separator: true as const },
    {
      label: t('stImportCsv', language),
      icon: <Upload size={11} className="text-[#4299e1]" />,
      onClick: () => onOpenImport(connId, db, table.name),
    },
    { separator: true as const },
    {
      label: t('stExportCsv', language),
      icon: <Download size={11} className="text-[#68d391]" />,
      onClick: () => exportData('csv'),
    },
    {
      label: t('stExportJson', language),
      icon: <Download size={11} className="text-[#68d391]" />,
      onClick: () => exportData('json'),
    },
    {
      label: t('stExportSqlInsert', language),
      icon: <Download size={11} className="text-[#68d391]" />,
    ...
    <div>
      <TreeRow
        ...
        onClick={() => {
          ...
          // 테이블 클릭 시 소속 연결/DB 도 함께 선택 — 툴바·쿼리 컨텍스트가 즉시 이 DB 를 가리키도록.
          setSelectedConn(connId)
          setSelectedDatabase(db)
          onTableSelect?.(connId, db, table)
        }}
      ...
      {showTriggers && (
        <div>
          <TreeRow
            ...
            label={triggersLoading ? t('stLoading', language) : `${t('stTriggers', language)} (${triggers?.length ?? 0})`}
...
function TriggerNode({
  ...
  const contextItems: ContextMenuOption[] = [
    { label: t('stShowDdl', language), icon: <Code size={11} />, onClick: showDDL },
    { label: t('stCopyName', language), icon: <Copy size={11} />, onClick: () => {
