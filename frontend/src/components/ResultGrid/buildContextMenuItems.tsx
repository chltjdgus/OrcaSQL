import { Plus, Copy, Eraser, ArrowUpRight, CornerDownLeft } from 'lucide-react'
...
export function buildContextMenuItems({
  ...
  onInsertRow, onDuplicateRow, onResetView, schemaMeta, onFkNavigate, language,
}: {
  rowIdx: number
  colIdx: number
  ...
  /** Phase 64: 클릭 셀 컬럼의 FK 관계 파생용 스키마 메타 */
  schemaMeta?: TableSchemaMeta | null
  /** Phase 64: FK 네비게이션 — 대상 테이블을 `column = value` 필터로 연다 */
  onFkNavigate?: (target: { table: string; column: string; value: unknown }) => void
  ...
  // Phase 64: FK 네비게이션 — 클릭 셀 컬럼의 정/역 FK 로 이동. non-NULL 셀 + 단일테이블 컨텍스트일 때만.
  const clickedCol = columns[colIdx]
  if (editCtx && onFkNavigate && clickedCol && cellVal !== null && cellVal !== undefined) {
    const outs = schemaMeta?.fksOut.get(clickedCol) ?? []
    const ins = schemaMeta?.fksIn.get(clickedCol) ?? []
    if (outs.length || ins.length) {
      items.push({ separator: true })
      for (const ref of outs) {
        items.push({
          label: `${t('fkNavGoTo', language)}: ${ref.refTable}`,
          icon: <ArrowUpRight size={11} className="text-[var(--color-accent)]" />,
          onClick: () => onFkNavigate({ table: ref.refTable, column: ref.refColumn, value: cellVal }),
        })
      }
      for (const ref of ins) {
        items.push({
          label: `${t('fkNavShowReferencing', language)}: ${ref.childTable}`,
          icon: <CornerDownLeft size={11} className="text-[var(--color-accent)]" />,
          onClick: () => onFkNavigate({ table: ref.childTable, column: ref.childColumn, value: cellVal }),
        })
      }
    }
  }
