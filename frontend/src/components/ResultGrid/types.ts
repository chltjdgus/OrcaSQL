...
/** FK 정방향 참조 대상 (자식 컬럼 → 부모 테이블/컬럼) */
export interface FkOutRef {
  refTable: string
  refColumn: string
}

/** FK 역방향 참조원 (부모 컬럼 ← 자식 테이블/컬럼) */
export interface FkInRef {
  childTable: string
  childColumn: string
}
...
export interface TableSchemaMeta {
  ...
  /** Phase 64: 정방향 FK — 현재 테이블의 FK 컬럼 → 참조 대상(부모). 셀에서 "참조 레코드로 이동". */
  fksOut: Map<string, FkOutRef[]>
  /** Phase 64: 역방향 FK — 현재 테이블의 피참조 컬럼 → 이를 참조하는 자식들. "참조하는 행 보기". */
  fksIn: Map<string, FkInRef[]>
}
