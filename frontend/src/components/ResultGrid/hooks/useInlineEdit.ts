...
export function useInlineEdit({
  ...
  /**
   * BugFix-DK — 편집 중 그리드 스크롤·창 리사이즈로 셀 위치가 바뀌어도
   * 팝오버(`TextAreaEditor`/`SetEditor`) 가 셀에 계속 정렬되도록 anchor 를 재계산.
   * `data-osql-cell-key` 로 현재 편집 td 를 매번 다시 lookup — 가상화로 인해 같은
   * 논리 셀이 새로운 DOM 노드로 재마운트될 수 있어 startEdit 시점 캡처한 ref 는 무효.
   * 셀이 가상화로 unmount 된 경우(쿼리 그리드 밖으로 멀리 스크롤) 는 마지막 위치 유지.
   */
  const recomputeAnchorRect = useCallback(() => {
    if (!editingCell) return
    const key = `${editingCell.rowIdx}-${editingCell.colIdx}`
    const el = typeof document !== 'undefined'
      ? document.querySelector(`[data-osql-cell-key="${key}"]`)
      : null
    if (el instanceof HTMLElement) {
      setEditAnchorRect(el.getBoundingClientRect())
    }
  }, [editingCell])
  ...
  const confirmEdit = useCallback(async () => {
    ...
    try {
      ...
      setEditingCell(null)
      toast.success(`${colName}${t('gridUpdateOkSuffix', language)}`)
      ...
      setEditingCell(null)
      toast.error(`${t('gridUpdateFailPrefix', language)}${e}`)
    } finally {
      setIsSaving(false)
    }
