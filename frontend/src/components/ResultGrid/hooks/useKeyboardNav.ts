...
/**
 * ResultGrid 키보드 네비게이션 (Phase 57 · Excel-style).
 *
 * **비편집 모드**
 * - `←↑↓→` : 한 셀 이동 (핀/숨김 컬럼 자동 skip — visibleColIdxList 기준)
 * - `Tab` / `Shift+Tab` : 다음/이전 셀. 행 끝 → 다음 행 첫 셀 (Excel wrap)
 * - `Enter` / `F2` : 편집 진입 (편집 가능 컬럼에 한해)
 * - `Escape` : 행에 dirty 가 있으면 롤백, 없으면 focus 해제
 * - `PageUp` / `PageDown` : viewport 행 수만큼 이동
 * - `Home` / `End` : 행 첫/끝 셀, `Ctrl+Home`/`Ctrl+End` : 전체 첫/끝
...
export function useKeyboardNav({
  ...
  /**
   * 셀이 viewport 가로 영역 안에 보이도록 컨테이너 scrollLeft 보정.
   * 좌측 sticky 영역(행번호 + 핀 컬럼) 뒤에 가려지면 왼쪽으로,
   * 우측 viewport 밖이면 오른쪽으로 스크롤. 핀 컬럼 셀은 sticky 라 보정 불필요(자동 통과).
   *
   * 가상화 row 가 아직 DOM 에 없을 수 있으므로 rAF 안에서 호출 — scrollToIndex 후
   * React 리렌더 + DOM 부착이 완료된 다음 셀을 찾아 보정한다.
   */
  /**
  ...
  }, [rowCount, visibleColIdxList.length, focusedRowIdx, beforeRowChange, setFocusedRowIdx, setFocusedColIdx, onSelectRow, rowVirtualizer, scrollCellIntoViewHorizontally])
  ...
  const onKeyDown = useCallback((e: React.KeyboardEvent) => {
    ...
    switch (e.key) {
      ...
      case 'ArrowLeft':
        e.preventDefault()
        if (pos > 0) void moveTo(r, visibleColIdxList[pos - 1])
        return
      case 'ArrowRight':
        e.preventDefault()
        if (pos < lastPos) void moveTo(r, visibleColIdxList[pos + 1])
        return
      case 'Tab': {
        e.preventDefault()
        let nextR = r
        ...
        if (nextR < 0 || nextR >= rowCount) return
        void moveTo(nextR, visibleColIdxList[nextPos])
        return
      }
      case 'Home':
        e.preventDefault()
        if (e.ctrlKey || e.metaKey) void moveTo(0, visibleColIdxList[0])
        else void moveTo(r, visibleColIdxList[0])
        return
      case 'End':
        e.preventDefault()
        if (e.ctrlKey || e.metaKey) void moveTo(rowCount - 1, visibleColIdxList[lastPos])
        else void moveTo(r, visibleColIdxList[lastPos])
        return
      case 'PageUp':
        e.preventDefault()
        void moveTo(r - getPageSize(), c)
        return
      case 'PageDown':
        e.preventDefault()
        void moveTo(r + getPageSize(), c)
