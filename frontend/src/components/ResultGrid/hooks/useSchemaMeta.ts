...
import type { TableSchemaMeta, FkOutRef, FkInRef } from '../types'
  ...
  useEffect(() => {
    ...
    void (async () => {
      try {
        ...
        // Phase 64: FK 플래그 + 정/역방향 관계 맵. GetForeignKeys 는 DB 내 모든 FK 를 반환하므로
        // 같은 결과에서 현재 테이블 기준 정방향(부모로)·역방향(자식으로) 양쪽을 파생한다.
        const fksOut = new Map<string, FkOutRef[]>()
        const fksIn = new Map<string, FkInRef[]>()
        for (const fk of fks ?? []) {
          if (fk.tableName === ctxTable) {
            addFlag(fk.columnName, 'FK')
            const arr = fksOut.get(fk.columnName) ?? []
            arr.push({ refTable: fk.refTableName, refColumn: fk.refColumnName })
            fksOut.set(fk.columnName, arr)
          }
          if (fk.refTableName === ctxTable) {
            const arr = fksIn.get(fk.refColumnName) ?? []
            arr.push({ childTable: fk.tableName, childColumn: fk.columnName })
            fksIn.set(fk.refColumnName, arr)
          }
        }

        const meta: TableSchemaMeta = { columns: colMap, flags: flagMap, fksOut, fksIn }
