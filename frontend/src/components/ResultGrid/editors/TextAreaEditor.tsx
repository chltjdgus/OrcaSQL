...
export function TextAreaEditor({
  ...
  // 인라인 모드: 팝오버로 표시
  if (isInline) {
    // BugFix-DK — viewport 가장자리 셀 편집 시 팝오버가 화면 밖으로 잘리던 문제.
    // width = max(cell, 300) 를 먼저 산출한 뒤 left 를 좌우 8px 마진 안으로 clamp,
    // 셀이 너무 좁아도 최소 300px 는 보장하고 viewport 보다 더 넓으면 viewport-16 까지 축소.
    const MARGIN = 8
    const posStyle: React.CSSProperties = anchorRect
      ? (() => {
          const viewportW = window.innerWidth
          const desiredWidth = Math.max(anchorRect.width, 300)
