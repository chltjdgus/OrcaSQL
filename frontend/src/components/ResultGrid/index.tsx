      ...
      {ctxMenu && (
        <ContextMenu
          ...
          items={buildContextMenuItems({
            ...
            // Phase 64: FK 네비게이션 — 대상 테이블을 `column = value` 필터로 열도록 App 에 이벤트 전파.
            schemaMeta,
            onFkNavigate: (editCtx && connId) ? (target) => {
              setCtxMenu(null)
              window.dispatchEvent(new CustomEvent('fk:navigate', {
                detail: { connId, db: editCtx.database, table: target.table, column: target.column, value: target.value },
              }))
            } : undefined,
