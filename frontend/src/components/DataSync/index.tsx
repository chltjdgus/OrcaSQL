...
export default function DataSync({ onClose }: Props) {
  ...
  async function runCompare() {
    if (!srcConn || !srcDB || !srcTable || !dstConn || !dstDB || !dstTable) {
      toast.error('소스와 대상 연결/DB/테이블을 모두 선택하세요')
      return
    }
    setComparing(true)
    setResult(null)
    ...
    try {
      ...
      if (res.diffs.length === 0) {
        toast.success('데이터가 동일합니다 — 차이 없음')
      } else {
      ...
      <div className="flex-1 overflow-hidden flex flex-col">
        ...
        {result && (
          <div className="flex border-b border-[var(--color-border)] bg-[var(--color-bg-secondary)] shrink-0">
            {(['diff', 'sql'] as const).map((tab) => (
              ...
              >
                {tab === 'diff' ? `Diff 목록 (${result.diffs.length})` : 'SQL 미리보기'}
              </button>
            ))}
          </div>
        )}
        ...
        {!result && !comparing && (
          <div className="flex flex-col items-center justify-center flex-1 gap-3 text-[var(--color-null)]">
            ...
            <p className="text-sm">소스/대상 테이블을 선택하고 비교를 시작하세요</p>
          </div>
        )}
        ...
        {comparing && (
          <div className="flex flex-col items-center justify-center flex-1 gap-3 text-[var(--color-text-muted)]">
            ...
            <p className="text-sm">데이터 비교 중…</p>
          </div>
        )}
            ...
            <div className="w-1/2 overflow-y-auto border-r border-[var(--color-border)]">
              {result.diffs.length === 0 ? (
                <div className="flex flex-col items-center justify-center h-full gap-2 text-[var(--color-text-muted)]">
                  ...
                  <p className="text-sm">데이터가 동일합니다</p>
                </div>
              ) : (
                <table className="w-full text-[11px] border-collapse">
                  <thead className="sticky top-0 bg-[var(--color-bg-secondary)] z-10">
                    <tr>
                      <th className="text-left px-3 py-1.5 text-[10px] text-[var(--color-text-muted)] font-semibold uppercase border-b border-[var(--color-border)] w-20">액션</th>
...
function DiffDetail({ diff }: { diff: DataDiffRow }) {
  ...
  return (
    <div className="p-3 space-y-3">
      ...
      <table className="w-full text-[11px] border-collapse">
        <thead>
          <tr className="border-b border-[var(--color-border)]">
            <th className="text-left px-2 py-1 text-[10px] text-[var(--color-text-muted)] font-semibold uppercase">컬럼</th>
            <th className="text-left px-2 py-1 text-[10px] text-[var(--color-accent)] font-semibold uppercase">소스</th>
            {diff.action === 'UPDATE' && (
              <th className="text-left px-2 py-1 text-[10px] text-[var(--color-error)] font-semibold uppercase">대상 (변경 전)</th>
            )}
          </tr>
        </thead>
        <tbody>
      ...
      {/* SQL 미리보기 */}
      <div>
        <p className="text-[10px] text-[var(--color-null)] mb-1 uppercase">생성 SQL</p>
