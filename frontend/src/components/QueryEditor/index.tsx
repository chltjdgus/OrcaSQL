...
export default function QueryEditor({ editorRef: externalRef, showHistory = false, onHideHistory, hideTabBar = false }: Props) {
  ...
  const handleEditorMount: OnMount = useCallback((editorInstance, monacoInstance) => {
    ...
    // Phase 63: 명령 팔레트 오픈 (Ctrl/Cmd+K · Ctrl/Cmd+P) — Monaco 가 Ctrl+K chord 를
    // 가로채므로 에디터 포커스 시엔 이 바인딩이 필수(window 핸들러는 미도달).
    editorInstance.addCommand(
  ...
  function runQuery(selectionOnly = false) {
    ...
    if (!connId) {
      toast.error(t('qeSelectConnFirst', language))
      return
    }
    ...
    if (!sql.trim()) {
      toast.error(t('qeEnterSql', language))
      return
    }
  ...
  async function handleCancel() {
    ...
    try {
      ...
      toast.success(t('qeCancelRequested', language))
    } catch {
      toast.error(t('qeCancelFail', language))
    }
  }
    ...
    <div className="flex flex-col h-full overflow-hidden">
            ...
            >
              {editingTabId === tab.id ? (
                ...
                <span
                  ...
                  title={t('qeTabRenameHint', language)}
                >
                  {tab.title}
                </span>
              )}
      ...
      {(activeTab?.connId ?? selectedConnId) && (
        <div className={`flex items-center gap-2 px-3 py-1 text-[10px] border-b shrink-0 ${connBarBg}`}>
          ...
          {activeTab?.database ?? selectedDatabase ?? t('qeNoDbSelected', language)}
        </div>
      )}
