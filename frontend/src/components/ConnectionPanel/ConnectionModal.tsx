...
/** ErrorKind에 맞는 원인 설명 (i18n) */
function describeError(result: TestConnResult, language: Language): { title: string; detail: string } {
  const msg = result.message ?? ''
  switch (result.errorKind) {
    case 'host':
      return { title: t('cmErrHostTitle', language), detail: t('cmErrHostDetail', language).replace('{msg}', msg) }
    case 'auth':
      return { title: t('cmErrAuthTitle', language), detail: t('cmErrAuthDetail', language).replace('{msg}', msg) }
    case 'database':
      return { title: t('cmErrDbTitle', language), detail: t('cmErrDbDetail', language).replace('{msg}', msg) }
    case 'ssh':
      return { title: t('cmErrSshTitle', language), detail: t('cmErrSshDetail', language).replace('{msg}', msg) }
    case 'proxy':
      return { title: t('cmErrProxyTitle', language), detail: t('cmErrProxyDetail', language).replace('{msg}', msg) }
    case 'tls':
      return { title: t('cmErrTlsTitle', language), detail: t('cmErrTlsDetail', language).replace('{msg}', msg) }
    default:
      return { title: t('cmErrOtherTitle', language), detail: msg }
  }
}
...
export default function ConnectionModal({ initial, defaultGroupId, groups = [], onSave, onClose }: Props) {
  ...
  const errorInfo = testResult && !testResult.ok ? describeError(testResult, language) : null
      ...
      <div className="bg-[var(--color-bg-tertiary)] border border-[var(--color-border)] rounded-lg shadow-2xl w-[500px] max-h-[90vh] flex flex-col">
        ...
        <div className="flex items-center justify-between px-5 py-4 border-b border-[var(--color-border)]">
          <h2 className="text-sm font-semibold text-[var(--color-text-primary)]">
            {initial ? t('cmEditTitle', language) : t('cmNewTitle', language)}
          </h2>
        ...
        <div className="flex border-b border-[var(--color-border)] px-5">
          {(['basic', 'ssh', 'proxy'] as const).map((tk) => (
            <button
              key={tk}
              onClick={() => setTab(tk)}
            ...
            >
              {tk === 'basic' ? t('cmTabBasic', language) : tk === 'ssh' ? t('cmTabSsh', language) : t('cmTabProxy', language)}
            </button>
          ))}
        </div>
        ...
        <form onSubmit={handleSubmit} className="flex-1 overflow-y-auto px-5 py-4 space-y-3">
          {tab === 'basic' && (
            <>
              <Field label={`${t('cmFieldName', language)} *`} required>
                <input
                  type="text"
                  value={cfg.name}
              ...
              <div className="grid grid-cols-3 gap-2">
                <Field label={t('cmFieldHost', language)} className="col-span-2">
                  <input type="text" value={cfg.host} onChange={(e) => set('host', e.target.value)} className={inputCls} />
                </Field>
                <Field label={t('cmFieldPort', language)}>
                  <input type="number" value={cfg.port} onChange={(e) => set('port', Number(e.target.value))} className={inputCls} min={1} max={65535} />
                </Field>
              </div>
              <div className="grid grid-cols-2 gap-2">
                <Field label={t('cmFieldUser', language)}>
                  <input type="text" value={cfg.user} onChange={(e) => set('user', e.target.value)} className={inputCls} />
                </Field>
                <Field label={t('cmFieldPassword', language)}>
                  <input type="password" value={cfg.password ?? ''} onChange={(e) => set('password', e.target.value)} className={inputCls} placeholder={t('cmPhKeychain', language)} />
                </Field>
              </div>
              <Field label={t('cmFieldDefaultDb', language)}>
                <DbCombobox
              ...
              <div className="grid grid-cols-2 gap-2">
                <Field label={t('cmFieldCharset', language)}>
                ...
                <Field label="TLS">
                  <div className="flex items-center h-8 gap-2">
                    ...
                    />
                    <label htmlFor="tls" className="text-xs text-[var(--color-text-muted)]">{t('cmTlsEnable', language)}</label>
                  </div>
                </Field>
              </div>
              {/* 그룹 & 색상 */}
              <div className="grid grid-cols-2 gap-2">
                {groups.length > 0 && (
                  <Field label={t('cmFieldGroup', language)}>
                    <select
                      value={cfg.groupId ?? ''}
                      onChange={(e) => set('groupId', e.target.value)}
                      className={inputCls}
                    >
                      <option value="">{t('cpUngrouped', language)}</option>
                      {groups.map((g) => (
                ...
                <Field label={t('cmFieldColorBadge', language)}>
                  <div className="flex items-center gap-1.5 h-8">
                    {CONN_COLORS.map((c) => (
                      <button
                        key={c || 'none'}
                        type="button"
                        title={c || t('cmNone', language)}
              ...
              {cfg.useSSH && (
                <>
                  ...
                  <div className="rounded bg-[var(--color-bg-secondary)] border border-[var(--color-border)] px-3 py-2 text-[10px] text-[var(--color-text-muted)] mt-1">
                    <span className="text-[var(--color-success)] font-medium">{t('cmTofuTitle', language)}</span>{t('cmTofuBefore', language)}
                    <code className="text-[var(--color-text-subtle)]">~/.orcasql/known_hosts</code>{t('cmTofuAfter', language)}
                  </div>
                </>
              )}
            </>
          ...
          {tab === 'proxy' && (
            <>
              <div className="rounded bg-[var(--color-bg-secondary)] border border-[var(--color-border)] px-3 py-2 text-[10px] text-[var(--color-text-muted)] space-y-0.5 mb-3">
                <p><span className="text-[var(--color-accent)] font-medium">SOCKS5</span>{t('cmProxySocks5Desc', language)}<code className="text-[var(--color-text-subtle)]">ssh -D 1080 user@server</code></p>
                <p><span className="text-[var(--color-accent)] font-medium">HTTP</span>{t('cmProxyHttpDesc', language)}</p>
              </div>
              <div className="flex items-center gap-2 mb-4">
                <input
                  type="checkbox"
                  ...
                  className="w-4 h-4 rounded"
                />
                <label htmlFor="useProxy" className="text-xs font-medium text-[var(--color-text-primary)]">{t('cmProxyUse', language)}</label>
              </div>
              {cfg.useProxy && (
                <>
                  <Field label={t('cmProxyType', language)}>
                    <select
                      value={cfg.proxyType}
                      onChange={(e) => {
                  ...
                  <div className="grid grid-cols-3 gap-2">
                    <Field label={t('cmProxyHost', language)} className="col-span-2">
            ...
            >
              {testResult.ok ? (
                <div className="flex items-center gap-2">
                  ...
                  <div>
                    <span className="font-medium text-[var(--color-success)]">{t('cmTestOk', language)}</span>
