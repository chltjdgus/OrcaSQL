...
export default function ConnectionPanel() {
  ...
  // 초기 로드
  useEffect(() => {
    GetSessionGroups().then(setGroups).catch(() => { toast.error('세션 그룹 로드 실패') })
  }, [setGroups])
  ...
  async function handleImportFile(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    try {
      ...
      setGroups(grps ?? [])
      toast.success(`${added}개 연결 가져오기 완료`)
    } catch (e) {
  ...
  async function handleDisconnect(connId: string) {
    try {
      ...
      toast.error(`해제 실패: ${e instanceof Error ? e.message : String(e)}`)
    }
  }
  ...
  async function handleDeleteGroup(grp: SessionGroup) {
    ...
    try {
      ...
      toast.success(`그룹 '${grp.name}' 삭제`)
    } catch (e) {
      toast.error(`그룹 삭제 실패: ${e}`)
    }
  }
  ...
  return (
    <div className="flex flex-col h-48 shrink-0">
      ...
      <div className="flex items-center justify-between px-3 py-2 border-b border-[var(--color-border)]">
        <span className="text-xs font-semibold text-[var(--color-text-muted)] uppercase tracking-wider">연결</span>
  ...
  return (
    <div className="relative">
      ...
      {showMoveMenu && (
        ...
        >
          <div className="px-2 py-1 text-[10px] text-[var(--color-null)] uppercase tracking-wider">그룹 이동</div>
          {/* 미분류로 이동 */}
          <button
