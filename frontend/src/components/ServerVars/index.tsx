...
export default function ServerVars({ connId, onClose }: Props) {
  ...
  function refresh() {
    activeQuery.refetch().catch(() => toast.error('새로고침 실패'))
  }
    ...
    <div className="flex flex-col h-full bg-[var(--color-bg-primary)] text-[var(--color-text-primary)]">
      ...
      <div className="flex-1 overflow-auto">
        {activeQuery.isLoading ? (
          <div className="flex items-center justify-center h-32 text-[var(--color-null)] text-xs">로딩 중...</div>
        ) : activeQuery.isError ? (
          <div className="flex items-center justify-center h-32 text-[var(--color-error)] text-xs">로드 실패</div>
        ) : (
...
function VarRow({ row }: { row: VariableRow }) {
  ...
  return (
    ...
    >
      ...
      <td className={`px-3 py-1.5 font-mono select-text ${
        ...
        {copied && <span className="ml-2 text-[var(--color-success)] text-[9px]">복사됨</span>}
      </td>
    </tr>
  )
}
