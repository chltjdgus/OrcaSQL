...
export default function App() {
  ...
  const cmdNavigateTable = useCallback((connId: string, db: string, table: TableInfo) => {
    ...
    // 트리 단일 클릭 + BugFix-DZ 와 동일: 이미 테이블 스코프 탭이면 유지, 아니면 Info 진입.
    setActiveToolTab((prev) => (prev === 'tableData' || prev === 'info') ? prev : 'info')
  }, [setSelectedConn, setSelectedDatabase])
                ...
                <SchemaTree
                  onTableSelect={(cId, db, table) => {
                    ...
                    pendingTableClickRef.current = window.setTimeout(() => {
                      ...
                      // 보고 있던 테이블 스코프 탭(Info/Data)을 유지 — 다른 테이블로 옮겨도
                      // 같은 탭을 계속 본다(Data 보다가 테이블 바꾸면 그 테이블 Data 표시).
                      // query workspace(null)·History 등 비-테이블 컨텍스트에서는 Info 로 진입(기존 기본).
                      setActiveToolTab((prev) => (prev === 'tableData' || prev === 'info') ? prev : 'info')
                      pendingTableClickRef.current = null
                    }, 220)
                  }}
