...
/**
 * 스키마 관련 TanStack Query 캐시 키 목록. DDL 실행 후 일괄 무효화한다.
 * Phase 61: 디자이너 메타('tableMeta')도 포함 — ALTER 후 다음에 디자이너를 열면 fresh 하게 로드.
 */
const SCHEMA_QUERY_KEYS = ['databases', 'tables', 'columns', 'procedures', 'functions', 'triggers', 'events', 'views', 'tableMeta'] as const
  ...
  const mutation = useMutation<QueryResult[], Error, ExecOptions>({
      ...
      const runMulti = async ({
        ...
        try {
          ...
          if (isConnectionError(msg)) {
            toast.loading(t('qxConnLostReconnecting', lang), { id: 'reconnect' })
            try {
              await Reconnect(connId)
              toast.success(t('qxReconnectedRerun', lang), { id: 'reconnect' })
              out = await callApi()
            } catch (reconnErr) {
              toast.error(`${t('toastReconnectFail', lang)} ${reconnErr}`, { id: 'reconnect' })
              throw reconnErr
            }
          } else {
            throw err
        ...
        if (out.failedIndex >= 0) {
          ...
          logMsg({
            kind: 'query',
            level: 'error',
            title: `Statement ${out.failedIndex + 1}${out.totalCount ? ` / ${out.totalCount}` : ''}${t('qxStatementFailedSuffix', lang)}`,
            detail: out.error,
            sql: out.failedSQL,
            stmtIndex: out.failedIndex,
            totalCount: out.totalCount,
          ...
          if (!out.remainingSQL || out.remainingSQL.trim() === '') {
            toast.error(`Statement ${out.failedIndex + 1}${t('qxStatementFailedSuffix', lang)}: ${out.error}`)
            return done
          }
    ...
    onSuccess: async (data, { tabId, connId }) => {
      ...
      if (threshold > 0 && elapsedSec >= threshold) {
        ...
        ShowNotification(t('qxNotifyDoneTitle', lang), `${data.length}${t('qxQueriesDoneSuffix', lang)} (${ms}ms)`).catch(() => {})
      }
      ...
      data.forEach((r) => {
        ...
        logMsg({
          ...
          level: 'success',
          title: isSelect
            ? `${(r.rows?.length ?? 0).toLocaleString()}${t('qxRowsReturnedSuffix', lang)}`
            : `Query OK, ${r.affected.toLocaleString()}${t('qxAffectedSuffix', lang)}`,
          sql: r.sql,
      ...
      if (data.length === 1) {
        ...
        if ((d.columns?.length ?? 0) > 0) {
          toast.success(`${(d.rows?.length ?? 0).toLocaleString()}${t('qxRowsReturnedSuffix', lang)} (${ms}ms)`)
        } else {
          toast.success(`${d.affected.toLocaleString()}${t('qxAffectedSuffix', lang)} (${ms}ms)`)
        }
        ...
        toast.success(`${data.length}${t('qxQueriesDoneSuffix', lang)} (${parts.join(', ')}) ${ms}ms`)
      }
    ...
    onError: (error, { tabId, sql }) => {
      const lang = useLanguageStore.getState().language
      const failMsg = error.message ?? t('qxExecFail', lang)
      const elapsedSec = (Date.now() - (startTimesRef.current.get(tabId) ?? Date.now())) / 1000
      startTimesRef.current.delete(tabId)
      const threshold = useSettingsStore.getState().settings.query.notifyThresholdSec
      if (threshold > 0 && elapsedSec >= threshold) {
        ShowNotification(t('qxNotifyFailTitle', lang), failMsg).catch(() => {})
      }
      updateTab(tabId, {
        isRunning: false,
        queryError: { sql, message: failMsg },
      })
      // BugFix-BN: Messages 세션 로그
      logMsg({
        kind: 'query',
        level: 'error',
        title: t('qxExecFail', lang),
        detail: failMsg,
        sql,
      })
      toast.error(failMsg)
