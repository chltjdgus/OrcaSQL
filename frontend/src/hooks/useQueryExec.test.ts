...
describe('useQueryExec', () => {
  beforeEach(() => {
    vi.clearAllMocks()
    // BugFix-DR: toast/logMsg 가 i18n 화되어 언어 의존 — CI 의 navigator.language 와 무관하게
    // 한국어 assertion(예: /행 영향/)이 유효하도록 'ko' 로 고정한다.
  ...
  it('단일 SELECT 성공 — updateTab 에 result 적재, success toast, logMsg success', async () => {
    ...
    // BugFix-DR: logMsg 제목도 i18n 화 — 'ko' 고정이므로 '행 반환'
    expect(logs[0].title).toMatch(/행 반환/)
  })
