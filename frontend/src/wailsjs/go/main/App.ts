...
// ─── Phase 65/66: DB 개요 대시보드 · 성능 인사이트 ──────────────────────────

export function GetDatabaseOverview(connID: string, database: string): Promise<DatabaseOverview> {
  return callGo<DatabaseOverview>('GetDatabaseOverview', connID, database)
}

export function GetPerformanceInsights(connID: string): Promise<PerformanceInsights> {
  return callGo<PerformanceInsights>('GetPerformanceInsights', connID)
}
