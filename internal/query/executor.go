...
func DetectQueryType(sqlStr string) QueryType {
	// BugFix-DY: 선행 주석/공백을 먼저 벗겨 실제 첫 키워드로 판정한다.
	// 종전엔 TrimSpace 만 해서 "/* x */ DROP ..." 나 "-- x\nDROP ..." 가 어떤 키워드에도
	// 매칭되지 않아 QueryTypeOther 로 오판됐고, MCP 정책 게이트에서 Other 가 AllowWrite 만으로
	// 통과해 DDL 보호(AllowDDL)를 우회할 수 있었다.
	trimmed := strings.ToUpper(stripLeadingCommentsAndSpace(sqlStr))
...
// stripLeadingCommentsAndSpace 문장 맨 앞의 공백과 SQL 주석(-- 라인, # 라인, /* */ 블록)을
// 반복 제거해 실제 첫 키워드가 드러나게 한다. 문자열 리터럴은 키워드보다 앞설 수 없으므로
// 선행 위치에서는 이 스킵이 안전하다(리터럴 내부의 -- 나 /* 를 오해할 여지 없음).
//
// 예외 — MySQL 실행 주석 `/*! ... */` 은 주석이 아니라 서버가 실제 실행하므로 여기서
// 스킵하지 않고 그대로 남긴다. 그 결과 '/' 로 시작해 어떤 키워드에도 매칭되지 않아 보수적으로
// QueryTypeOther 로 귀결되고, MCP 정책 계층(CheckExecutePolicy)이 별도로 거부한다.
func stripLeadingCommentsAndSpace(s string) string {
	for {
		...
		case strings.HasPrefix(s, "/*"):
			...
			if end < 0 {
				return s // 미닫힘 블록주석 — 그대로 반환(판정은 Other 로 귀결)
			}
			s = s[end+2:]
		case isDashLineComment(s):
			if nl := strings.IndexByte(s, '\n'); nl >= 0 {
				s = s[nl+1:]
			} else {
				return "" // 주석만 있고 개행 없이 끝
			}
		case strings.HasPrefix(s, "#"):
			if nl := strings.IndexByte(s, '\n'); nl >= 0 {
				s = s[nl+1:]
...
// isDashLineComment MySQL 라인 주석 "-- " 시작인지 판정한다.
// MySQL 은 "--" 다음에 공백/제어문자/EOL 이 와야 주석으로 취급한다("a--b" 는 뺄셈).
func isDashLineComment(s string) bool {
