...
func TestCheckExecutePolicy(t *testing.T) {
	tests := []struct {
		...
		// BugFix-DY 벡터 A — 선행 주석으로 DDL 을 Other 로 숨겨 AllowWrite 만으로 통과하던 우회.
		{"comment-hidden ddl blocked with write", Config{AllowWrite: true}, "/* x */ DROP TABLE t", true},
		{"line-comment-hidden ddl blocked with write", Config{AllowWrite: true}, "-- x\nDROP TABLE t", true},
		{"comment-hidden ddl ok with ddl flag", Config{AllowDDL: true}, "/* x */ DROP TABLE t", false},
		{"comment before select still ok", Config{}, "/* c */ SELECT 1", false},

		// BugFix-DY 벡터 C — 다중 문장으로 SELECT 뒤에 DDL/DML 을 숨기던 우회.
		{"multi statement blocked (select+ddl)", Config{AllowWrite: true, AllowDDL: true}, "SELECT 1; DROP TABLE t", true},
		{"multi statement blocked (select+select)", Config{}, "SELECT 1; SELECT 2", true},
		{"single select trailing semicolon ok", Config{}, "SELECT 1;", false},
		{"semicolon inside string literal ok", Config{}, "SELECT ';'", false},

		// BugFix-DY 실행 주석 — /*! ... */ 은 서버가 실제 실행하므로 거부.
		{"executable comment blocked", Config{AllowWrite: true}, "/*!40000 DROP TABLE t */", true},
		{"executable comment blocked even with ddl", Config{AllowDDL: true}, "/*!40000 DROP TABLE t */", true},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
